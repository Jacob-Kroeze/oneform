...
// fix a bit of the fileupload plugin
    ...
    if (remote_schema){
        options = remote_schema.options
    }
    if (!remote_schema){
    var options = {
        "formName": "no-name-demo",
        "fields": {
            "email": {
                "type": "text",
                "label": "Email Address"
...
//    $(".save-button").off().click(function() {
    $(".save-button").click(function() {
        ...
        if (options)
        {
//            config.options = options; // forcing simpler options.
            config.formName = options.formName
            config.options = {"formName": config.formName}
        ...
        var c = {
            method: "post",
            url: Alpaca.FORM_ACTION_ENDPOINT,
            data: configString,
            headers: {},
...
$(document).ready(function() {
    // initiate schema var
    // closure around ajax call
    var getSchema = function(url, csrf){
        //call out to api to get schema for this tablename.
        var getSchema = function(url, callback){
            $.ajax({
                "method": "GET",
                "url": url,
                "headers": {},
                "async": false, // async:false let's this work
                success: function(data, status, xhr){
                    callback(data);
                }
            });
        };
        // callback to set schema var
        getSchema(Alpaca.SCHEMAS_URL, function(data){
            console.log(data);
            remote_schema = data;
        });
    };
    // closure around callback
    var setSchema = function(){
        if (Alpaca.SCHEMAS_URL)
        {
            getSchema(Alpaca.SCHEMAS_URL, csrfToken);
        }
    };
    // do it.
    remote_schema = null;
